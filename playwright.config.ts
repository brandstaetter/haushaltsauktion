 ...
 *
 * `E2E_EXTERNAL_SERVERS=true` schaltet den `webServer`-Block unten ab: die
 * CI (.github/workflows/deploy.yml, "e2e"-Job) startet API und Web dort
 * bereits selbst als echte Docker-Container (Wegwerf-Stack aus den
 * tatsächlich gebauten Images, nicht `tsx`/Vite-Dev-Server) und wartet über
 * deren Healthchecks, bevor dieser Lauf beginnt — `global-setup.ts` (Seed)
 * läuft in beiden Modi unverändert.
 */
...
const EXTERNAL_SERVERS = process.env.E2E_EXTERNAL_SERVERS === 'true';
