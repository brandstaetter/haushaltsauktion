...
- Implementiert in `.github/workflows/restore-drill.yml` (`schedule: cron` montags 04:00 UTC + `workflow_dispatch`), läuft komplett im CI-Runner, ohne neue AWS-Ressourcen:
  ...
  5. Frische-Prüfung: das jüngste Backup darf höchstens `max_backup_age_days` (Default 3, per `workflow_dispatch`-Eingabe überschreibbar) alt sein. Backups laufen nächtlich, der Default toleriert also zwei ausgefallene Nächte. Die Prüfung läuft **nach** dem Restore — ein veralteter, aber noch einspielbarer Dump macht den Drill damit rot (ein toter `backup-db.timer` kann sich nicht hinter einem alten Dump verstecken), ohne dass die Information „der Restore selbst funktioniert“ verloren geht.
  6. Bei Fehlschlag: Workflow schlägt fehl → GitHub-Standardbenachrichtigung per E-Mail an die Repo-Eigentümer:in (ausreichend bei dieser Nutzerzahl, kein Slack/PagerDuty nötig).
- Findet Schritt 1 gar kein Objekt unter `backups/`, bricht der Workflow mit einer expliziten Meldung samt Runbook-Schritten in der Job-Summary ab (`systemctl status backup-db.timer`, `journalctl -u backup-db.service`, manueller Skriptlauf). Häufigste Ursachen: `BACKUP_S3_BUCKET` fehlt in der `.env` der Instanz, die Unit-Dateien wurden nie nach `/etc/systemd/system/` kopiert und aktiviert, oder die AWS-Zugangsdaten liegen nicht unter `/root/.aws/credentials` — `backup-db.service` setzt kein `User=`, läuft also als root und sieht die Credentials des Deploy-Users nicht.
